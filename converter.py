...
def oklch_to_hex(L, c, h_degrees):
    """
    Convert OKLCH(L, c, h) to an sRGB hex string (#RRGGBB).
    Formula based on Björn Ottosson’s Oklab post:
    ...
    b_lin = (-0.0045160938 * l_) + (-0.1536166770 * m_) + ( 1.1586886638 * s_)

    # 5) Apply sRGB gamma correction
    def gamma_encode(u):
        if u < 0.0:
            u = 0.0
        if u <= 0.0031308:
    ...
    # Clamp and convert to 0–255 integer
...
# ----------------------------------------------------------------------------
# Here are all your variables with "color-" stripped from the name.
# (Keeping exactly the same order you provided.)
# ----------------------------------------------------------------------------

variables = [
("base-100",           1,     0,       0),
("base-200",           0.98,  0.002,  247.839),
("base-300",           0.96,  0.003,  264.542),
("base-content",       0.21,  0.034,  264.665),
("primary",            0.27,  0.033,  256.848),
("primary-content",    1,     0,       0),
("secondary",          0.77,  0.152,  181.912),
("secondary-content",  0.27,  0.046,  192.524),
("accent",             0.85,  0.199,   91.936),
("accent-content",     0.26,  0.051,  172.552),
("neutral",            0.92,  0.006,  264.531),
("neutral-content",    0.21,  0.034,  264.665),
("info",               0.93,  0.032,  255.585),
("info-content",       0.42,  0.199,  265.638),
("success",            0.95,  0.052,  163.051),
("success-content",    0.43,  0.095,  166.913),
("warning",            0.95,  0.038,   75.164),
("warning-content",    0.47,  0.157,   37.304),
("error",              0.93,  0.032,   17.717),
("error-content",      0.44,  0.177,   26.899),
...
# Optional: also keep black/white from your original definition
# (They are already #000/#fff, but let's just keep them verbatim.)
static_hex = {
    "black": "#000000",
    "white": "#ffffff",
}
...
# 1) Convert all OKLCH entries
for (short_name, L, c, h) in variables:
    hex_color = oklch_to_hex(L, c, h)
    print(f"  --{short_name}: {hex_color};")

# 2) Print black/white exactly as given
for name, val in static_hex.items():
    print(f"  --{name}: {val};")
